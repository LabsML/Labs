...
def createSparseTable(dataset_filename, ntype=np.float64):
    ...
    try:
        ...
        print("Unable to open file '{}'".format(dataset_filename))
...
def computeOutBlocks(nBlocks, dataBlock, dataBlockPartition):
    ...
    for i in range(nRows):
        for j in range(int(rowOffsets[i] - 1), int(rowOffsets[i + 1] - 1)):
            for k in range(1, nBlocks + 1):
                if dataBlockPartition[k - 1] <= colIndices[j] - 1 and colIndices[j] - 1 < dataBlockPartition[k]:
    ...
    for i in range(nBlocks):
        indicesTable = HomogenNumericTable(1, int(nNotNull[i]), NumericTableIface.doAllocate, ntype=np.intc)
...
def computePartialModelBlocksToNode(nBlocks,
                                    dataTable,
                                    transposedDataTable,
                                    usersPartition, itemsPartition,
                                    usersOutBlocks,
                                    itemsOutBlocks):
    usersPartition[0] = 0
    itemsPartition[0] = 0
...
def printALSRatings(usersOffset, itemsOffset, ratings):
